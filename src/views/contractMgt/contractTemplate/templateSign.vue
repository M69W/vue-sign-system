<template>
    <div class="template-sign">
        <div class="inner">
            ...
            <section class="container container-sign">
                ...
                <div class="sign-list">
                    ...
                    <div class="sign-setting" v-if="signatoryWay !== '02' && !isFill">
                        <h2>签章设置</h2>
                ...
                <!-- 文件签署区 -->
                <div class="files" @mousewheel.stop="fileContainerWheel" @DOMMouseScroll.stop="fileContainerWheel">
                    <el-scrollbar>
                        <div class="img-wrap" ref="fileItemWrap">
                            <div
                                v-for="cloudItem in cloudSignList"
                                :key="cloudItem.devPictureId"
                                class="sign-wrap"
                                ref="signWrap"
                            ...
                            <div
                                ...
                                v-show="
                                    ...
                                    item.fileCode === fileCode && signatoryWay === '01'
                                "
                                :style="{
                            ...
                            <div
                                ...
                                v-for="(item, index) in templateField"
                                :key="index"
                                :style="{
                                    left: item.left + 'px',
                                    top: item.top + 'px'
                                }"
                                v-show="item.fileCode === fileCode &&
                                    item.signatureStartPage === pageNumber && 
                                    signatoryWay === '01'
                                "
                            >
                ...
                <div class="file-list">
                    ...
                    >
                        <el-tab-pane :label="`${isFile ? '签发' :'合同'}文件`" name="first">
                            <el-scrollbar>
                                <div
                                    class="file-item"
        ...
        >
            ...
            <div class="multiple-sign" v-else>
                <el-form ref="multiplePageNo" :model="formRule">
                    <el-form-item prop="pageNo" :rules="pageNoRule">
                        ...
                        v-model.trim="formRule.pageNo"></el-input>
                    </el-form-item>
                </el-form>
            </div>
        ...
        >
            ...
            >
                <el-form-item label="密码" prop="password">
                    <el-input
                        ...
                        @keyup.native.enter="signPwConfirm('signPwForm')"
                    ></el-input>
                </el-form-item>
            </el-form>
            ...
            <div class="no-sign-paw" v-else>
                未设置，<span @click="setSignPassword">立即设置</span>
            </div>
        </el-dialog>

        <!-- 验证码验证 -->
        <sign-password-dialog 
            ref="signPasswordDialog" :isSign="isSign"
            @submitSignPassword="submitSignPassword"
            @useSignPassword="dialogInputSignPassword=true"></sign-password-dialog>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        signTitle() {
            let title = ''
            if (this.signatoryWay === '02' || this.isFill) {
                title = '立即发送';
            }else if (
                this.signatoryWay === '01' &&
                this.contractForm.templateSignatories.length > 1
            ) {
                title = '签署并发起';
            }else {
                title = '完成签署'
    ...
    mounted() {
        ...
        // -----------------------------------------------------------------
        let contractForm = sessionStore.get('contractInfo');
    ...
    methods: {
        getControls() {
            this.templateField = this.contractForm.templateField.filter((v) => {
                if (
                    v.signType !== '01' &&
                    v.signType !== '02' &&
            ...
            this.isFill = !!this.contractForm.templateField.find(
                (v) =>
                    v.signType !== '01' &&
                    v.signType !== '02' &&
                    v.signatoryId !== this.userInfo.id
            ...
            if (this.isFill) {
                ...
                this.setPosArr = this.contractForm.templateField.filter(
                    (v) =>
            ...
            this.$ukeyApi('getKeyCertInfo')
                ...
                .then((res) => {
                    ...
                    if (this.successUKeyCheckCode(data)) {
                        ...
                        this.caType = keyType;
                        if (keyType === 'GDCA' || keyType === '公安三所CA' || driverType === 'gass') {
                            this.caType === 'GDCA';
                            // data.info.forEach(v=>{
                            //     if (v.item_type == 21) {
        ...
        getKeyInfo(check) {
            this.$ukeyApi('getKeyCertInfo')
                ...
                .then((res) => {
                    ...
                    if (this.successUKeyCheckCode(data)) {
                        data = data.info
                        let sealCode = data.find(v => v.item_type === 3).item_info
                        this.ukeySealCode = sealCode
                        this.certOID = data.find(v => v.item_type === 15).item_info
                        this.sealImgData = data.find(v => v.item_type === 20).item_info
                        this.gmstamp = data.find(v => v.item_type === 29) && data.find(v => v.item_type === 29).item_info || ''                        
                        this.isSM2 = data.find(v => v.item_type === 15).item_info === '1.2.156.10197.1.501'
                        let item = this.ukeySeal.find(v => v.pictureCode === sealCode)
                        let ukeySealItem = item || {}
                        item && (ukeySealItem.picturePath = 'data:image/png;base64,' + data.find(v => v.item_type === 20).item_info);
                        this.sealImgWidth = data.find(v => v.item_type === 21).item_info
                        this.sealImgHeight = data.find(v => v.item_type === 22).item_info
        ...
        async ukeySignFn() {
            ...
            for (let i = 0; i < ukeyList.length; i++) {
                ...
                if(!res) return
                let res1 = await this.$ukeyApi('signbydigestdata').post('postQuery', {
                    keyid: 0,
            ...
            this.ukeySignList.forEach((v) => {
                ...
                if (v.signatureMethod == '02') {
                    ...
                    pageNo.forEach(v=>{
                        ...
                        multipleSignPos.push(Object.assign({}, signPos, {startPage,endPage}))
                    })
                ...
                if (index < 0) {
                    ...
                    ukeySignList.push({
                        ...
                        signInfo: [{
                            ...
                            sealImgType: 'png',
                            signPosInfo: v.signatureMethod == '02' ? multipleSignPos : [signPos]                            
                        }]
                    });
                } else {
                    // 有就推入
                    if (v.signatureMethod == '02') {
                        multipleSignPos.forEach(v=> ukeySignList[index].signInfo[0].signPosInfo.push(v))
                    }else {
                        ukeySignList[index].signInfo[0].signPosInfo.push(signPos);
                    }
                }
            });
            return ukeySignList;
        ...
        addEventSignMove() {
            ...
            $('.container-sign').on('mousedown', '.seal-list-item', function(
                ...
                _self.shadowSignType = e.currentTarget.dataset.picturetype || '01';
                ...
                _self.nowSealWidth = (isUkey ? _self.ukeyWidth : target[0].dataset.picturewidth * 72 / 25.4) * _self.scale
                _self.nowSealHeight = (isUkey ? _self.ukeyHeight : target[0].dataset.pictureheight * 72 / 25.4) * _self.scale
                ...
                _self.shadowY =
                    sealToContY + (_self.shadowSignType==='01' ? 0 : 50) - (_self.nowSealHeight - targetHeight) / 2;
                ...
                $(document).bind('mousemove', function(e) {
                    let diffX =
                        e.pageX - contDocumentPosObj.left - _self.nowSealWidth / 2;
                    let diffY =
                        e.pageY - contDocumentPosObj.top - (_self.shadowSignType==='01' ? _self.nowSealHeight : 50) / 2;
                    // 右边界
                    let diffXmax =
                    ...
                    // 下边界
                    let diffYmax =
                        containerDom.clientHeight - (_self.shadowSignType==='01' ? _self.nowSealHeight : 50) - 11;
                    if (diffX <= 0) {
                        diffX = 0;
                    }
                    if (diffX >= diffXmax) {
                    ...
                    let addSignPosX =
                        diffX -
                        (getOffset($('.img-wrap')[0]).left - contDocumentPosObj.left) - 1;
                    let diffFileWrapFile =
                        ($('.img-wrap')[0].clientWidth - fileDom.clientWidth) / 2;
                    if (addSignPosX < diffFileWrapFile) {
                        addSignPosX = diffFileWrapFile;
                    }
                    addSignPosX += $('.files .el-scrollbar__wrap')[0].scrollLeft
                    let addSignPosY = diffY - 11 + $('.files .el-scrollbar__wrap')[0].scrollTop;
        ...
        addCloudSign(pictureCode, addSignPosX, addSignPosY, isUkey = false) {
            ...
            let width = isUkey ? this.ukeyWidth * this.scale : this.nowSealWidth;
            let height = this.shadowSignType === '02' ? 50 : isUkey
            ...
            let venderCloudItem = Object.assign({}, cloudSignItem, {
                ...
                signSubmitPosX:
                    (addSignPosX + width / 2) / this.scale,
                signSubmitPosY:
        ...
        dialogSure() {
            ...
            if (this.typeActive === '02') {
                ...
                if (
                    ...
                    this.formRule.coverPageNum > this.formRule.endPageNo - this.formRule.startPageNo + 1
                ) {
                    return this.$message.warning({
        ...
        resetDialogChooseType() {
            ...
            this.$nextTick(()=> this.$refs.multiplePageNo && this.$refs.multiplePageNo.clearValidate())
        },
        ...
        setStyle() {
            this.setPosArr.forEach((v) => {
                if (v.fileCode === this.fileCode && !this.fileSizeList[this.fileCode].scale) {
                    let posX = (v.signatureCoordinateX + 6) * this.scale + this.signWidth / 2
                    let posY = (v.signatureCoordinateY + 6) * this.scale + this.signWidth / 2
            ...
            imgWrapDom.on('mousedown', '.sign-wrap', function(e) {
                ...
                $(document).bind('mousemove', function(e) {
                    ...
                    _self.cloudSignList[dragCloudItemIndex].signSubmitPosY =
                        (fileHInner - top - target.outerHeight() / 2) /
                        _self.scale;
                    _self.cloudSignList[dragCloudItemIndex].y =
        ...
        submitSign() {
            ...
            // 仅他人签署或需他人填写控件，不要印章直接发起
            if (this.isFill || this.signatoryWay === '02') {
                return this.onlyOtherSign()
            }
            ...
            let signFileList = [...new Set(this.cloudSignList.map(v=>v.fileCode))];
        ...
        async onlyOtherSign(ukeySignList = []) {
            console.log('onlyOtherSign STRT ... \n---------------------');

            let _self = this;
            let compactFieldInfoReqVoList = []; // 签名域信息集

            let templateSignReqVoS; // 签章信息
            // if (this.signatoryWay === '01') {
            //     }
            // 取云签章信息
            let cloudSignList = this.cloudSignList.filter((v) => !v.isUkey);
            templateSignReqVoS = cloudSignList.map((v) => {
                ...
                return {
                    ...
                    signatureCoordinateX: v.signSubmitPosX / v.scale, // 签署坐标X轴 ,
                    signatureCoordinateY: this.fileSizeList[v.fileCode].height - v.signSubmitPosY / v.scale, // 签署坐标Y轴 ,
            ...
            for (const v of templateField) {
                ...
                // ) {
                //     this.contentArr = [];
                //     let oneline = Math.floor(
                //         (2 * v.width) / this.wordFontSizeToPx(v.fontSize)
                //     ); //一行可显示字数
                //     subString(v.content, 0, oneline);
                // }
            }
            // templateField.forEach((v) => {
            //     v.compactFileCode = v.fileCode;
            // });
            templateSignReqVoS = templateSignReqVoS.concat(templateField);
            // console.log(templateSignReqVOS)
            // templateSignReqVOS.forEach(v=> v.signUser = v.signatoryId)

            // TODO 完成签署BTN >>> 异步请求
            this.contractForm.templateSignatories.forEach((v) => {
                v.type = v.type || (v.sign === 1 ? '01' : '02');
            });
            let {compactTheme,contractFileList,templateId,remark,signDeadline,signWay,signatoryWay,validityEndDate} = this.contractForm
            if (this.$route.query.renew) {
                contractFileList.forEach(v=> {
            ...
            let sendData = Object.assign(
                {
                    ...
                    signDeadline, // 签署截止日期
                    signInfos: this.contractForm.templateSignatories, // 签署信息---签署人列表
                    // signName: '', // 签署人(自签)
                    signWay, // 签署方式（01：无序签；02：顺序签署；03：每人单独签署；）
                    signatoryWay: this.isFill ? '02' : signatoryWay, // 签署对象（01：我需要签署；02：仅需他人签署；）
                ...
                {
                    compactFieldInfoReqVoList, // 签名域信息集
                    templateSignReqVoS, // 签章信息
                    contractId: this.$route.query.contractId || '',
                    orgId: this.userInfo.enterpriseId, // 所属组织Id ,
                    type: this.isFile ? '01' : '02', //  类型（01:文件管理，02:合同）
                    contentType: 'json',
                    isSiteSign: '0'
                }
            );
            ...
            const url = this.isFile ? 'useFileTemplate' : 'signByTemplate'
            ...
            console.log(res);
            if (!res) return;
            const msg = this.isFile ? '发起文件成功！' : '发起合同成功！'
            const redirect = this.isFile ? '/fileCenter/fileManage' : '/contractMgt/index'
        ...
        adsorbSignToPos(
            ...
            for (const item of this.setPosArr) {
                if (
                    item.signatureStartPage === this.pageNumber &&
                    left + cloudSignItem.width / 2 > item.posX - this.signWidth * 0.2 &&
                    left + cloudSignItem.width / 2 < item.posX + this.signWidth * 0.2 &&
                    top + cloudSignItem.height / 2 > item.posY - this.signWidth * 0.2 &&
                    top + cloudSignItem.height / 2 < item.posY + this.signWidth * 0.2 &&
                ...
                flag = signInPosArr.find((v) =>{
                    if (v.isUkey) {
                        return (
                            ...
                            v.pageNo === this.setPosArr[i].signatureStartPage &&
        ...
        // 校验控件内容
        checkTemplateField() {
            let noFill = this.templateField.find(
                (v) => !v.content && v.isChoose === 1
            );
            if (noFill) {
                this.$message.warning('请为动态字段输入内容');
                return false;
            }
            ...
            //         v.signatureCoordinateY =
            //             1 -
            //             (this.wordFontSizeToPx(v.fontSize) +
            //                 5 +
            //                 v.signatureCoordinateY) /
            //                 this.heightList[v.fileCode];
            //         v.signatureCoordinateX = v.signatureCoordinateX / 595;
            //     } else {
            //         let dom = $('.el-select') || $('.el-date-editor');
            //         v.signatureCoordinateY =
            //             1 -
            //             (dom.height() + v.signatureCoordinateY) /
            //                 this.heightList[v.fileCode];
            //         v.signatureCoordinateX = v.signatureCoordinateX / 595;
            //     }
            // });
        ...
        submitSignPassword() {
            if (this.isSign) {
                ...
                this.isSignPassword = sessionStore.get('isSignPassword') === 'true' || false
            }
        },
        // 取消输入签署密码
        signPwCancel() {
        ...
        signPwConfirm(formName) {
            this.$refs[formName].validate(async (valid) => {
                if (!valid) return;
                let res = await this.$api('verifySignPassword').post('postQuery',{},{password:this.signPwForm.password});
                if(!res) return
                console.log(res);
    ...
    beforeRouteLeave(to, from, next) {
        console.log(to);
        if (
            to.path !== '/contractMgt/uploadTemplate' &&
            to.path !== '/fileCenter/uploadTemplate' 
        ) {
