            ...
            <section class="container container-sign">
                ...
                >
                    ...
                    <div class="sign-pos-wrap">
                        <div
                            class="sign-pos"
                            :class="{'no-border': shadowPosSignType === '02'}"
                            :style="{
                                width: signWidth + 'px',
                                height: (shadowPosSignType === '01' ? signWidth : 50) + 'px'
                            }"
                        >
                            {{shadowPosSignType === '01'?'印章':'签名'}}位置
                        </div>
                    </div>
                </div>
                ...
                <div class="sign-list">
                    ...
                    <div class="sign-setting" v-if="isFirstSign">
                        <h2>签章设置</h2>
                    ...
                    <!--  仅需他人签署（添加签署人后的功能） -->
                    <div
                        v-if="contractForm.signInfos.filter(v=>!v.self&&v.type==='01').length >= 1"
                        class="set-pos"
                        :class="{ 'reset-pos': isFirstSign }"
                    >
                        ...
                        <div class="pos-content">
                            ...
                            <div class="pos-sign" data-signtype="01">
                            ...
                            <div class="pos-sign" data-signtype="02">
                                <i class="el-icon-edit"></i>
                                <span class="pos-sign-word">签名</span>
                                <span class="pos-name">{{
                                    posSignPeople
                                }}</span>
                            </div>
                        ...
                        <div class="img-wrap" ref="fileItemWrap">
                                ...
                                >
                                    ...
                                    <div class="sign-pos-wrap">
                                        <div
                                            class="sign-pos"
                                            :class="{'no-border':item.signType==='02'}"
                                            :style="{
                                                width: signWidth + 'px',
                                                height: (item.signType==='01'?signWidth:50) + 'px'
                                            }"
                                        >
                                            {{item.signType==='01'?'印章':'签名'}}位置
                                        </div>
                                    </div>
                                </div>
                                <img
...
export default {
    ...
    data() {
        return {
            isViaProcess: this.$route.query.isViaProcess == 1,
            ...
            fileHeight: 0,
            totalPage: 0,
            standerdSignWidth: (40 * 72) / 25.4, // 标准章宽度（40mm * DPI(72) / 1 inch）
            imgList: [],
            event: false, // 避免冲入绑定
            signPosX: 0, // 验证签章位置用
            signPosY: 0,
            ...
            sealImgWidth: '',
            sealImgHeight: '',
            nowSealWidth: 0, // 当前操作印章的宽度
            nowSealHeight: 0, // 当前操作印章的高度
            fileSizeList: {}, // 文件尺寸对象
            isOfd: false, // 当前是否是OFD文件
            isSM2: false, // 是否是国密ukey
            isFirstSign: false, // 是否本人第一个签署
            signPwRules: {
                password: [
                    {
                        required: true,
    ...
    mounted() {
        ...
        if (contractForm) {
            ...
            if (this.signatoryWay) {
                this.optionsSignPeople = this.contractForm.signInfos.filter(
                    (v) => v.type === '01' && !v.self
                );
            }
        }
        ...
        let firstSignMan = this.contractForm.signInfos.filter(
            (v) => v.type === '01' || v.type === '03'
        );
        this.isFirstSign =
            (this.contractForm.signWay !== '02' &&
                this.signatoryWay === '01') ||
            (this.contractForm.signWay === '02' && firstSignMan[0].self);
    ...
    watch: {
        ...
        scale(val) {
            // if (this.contractForm.compactFieldInfoReqVOList && !this.setPosArr.length) {
            ...
            //                 signType: v.signType,
            //                 left: v.signatureCoordinateX * val - this.signWidth / 2 - 10,
            //                 top: v.signatureCoordinateY * val - this.signWidth / 2 - 35
            //             };
            //         }
            //     );
            // }
    ...
    methods: {
        ...
        setSignTitle() {
            if (!this.isFirstSign) {
                ...
                this.contractForm.signInfos.filter((v) => v.type === '01')
                    .length > 1
            ) {
        ...
        addEventSignPosMove() {
            ...
            $('.container-sign').on('mousedown', '.pos-sign', function(e) {
                ...
                _self.shadowPosSignType =
                    e.currentTarget.dataset.signtype || '01';
                ...
                let signPosShadow_height =
                    (_self.shadowPosSignType === '01' ? _self.signWidth : 50) +
                    41;
                let sealToContX =
                ...
                $(document).bind('mousemove', function(e) {
                    ...
                    // 下边界
                    let diffYmax =
                        containerDom.clientHeight - signPosShadow_height - 10;
                    if (diffX <= 0) {
                        diffX = 0;
                    }
                    if (diffX >= diffXmax) {
                    ...
                    let addSignPosX =
                        diffX -
                        (getOffset($('.img-wrap')[0]).left -
                            contDocumentPosObj.left);
                    // (fileDocumentPosObj.left - contDocumentPosObj.left);
                    let diffFileWrapFile =
                        ($('.img-wrap')[0].clientWidth - fileDom.clientWidth) /
                        2;
                    // (fileWrapDom.clientWidth - fileDom.clientWidth) / 2;
        ...
        /**
         * 添加指定签章位置
         * @param {Number} addSignPosX  - 横坐标
         * @param {Number} addSignPosY  - 纵坐标
         */
        addPosSignWrap(addSignPosX, addSignPosY) {
            ...
            this.setPosArr.push({
                ...
                devId: Date.now(),
                signType: this.shadowPosSignType, // 位置类型(01:印章；02:签名；03:日期) ,
                posX: (addSignPosX + 10 + this.signWidth / 2) / this.scale,
                posY: (addSignPosY + 35 + this.signWidth / 2) / this.scale,
        ...
        chooseSignType(typeNum) {
            ...
            if (this.signatoryWay === '02' && typeNum !== '01') {
                this.$message.warning('指定位置签章不支持多页或骑缝签署！'); //  待改进，此时应直接置灰其他签章类型
        ...
        setStyle(fileUrl) {
            if (
                this.contractForm.compactFieldInfoReqVOList &&
                !this.setPosArr.length
            ) {
                this.setPosArr = this.contractForm.compactFieldInfoReqVOList.map(
                    (v, i) => {
                        return {
                            ...
                            fileCode: v.compactFileCode,
                            signType: v.signType,
                            left: v.signatureCoordinateX,
                            top: v.signatureCoordinateY
                        };
                    }
                );
            }
            this.setPosArr.forEach((v) => {
                if (
                    ...
                    this.$set(
                        v,
                        'left',
                        v.posX * this.scale - this.signWidth / 2 - 10
                    );
                    this.$set(
                        v,
                        'top',
                        v.posY * this.scale - this.signWidth / 2 - 35
                    );
                }
            });
            !this.fileSizeList[this.fileCode].scale &&
        ...
        mouseMoveSetPos() {
            ...
            imgWrapDom.on('mousedown', '.sign-setpos', function(e) {
                ...
                $(document).bind('mousemove', function(e) {
                    ...
                    _self.setPosArr[moveIndex].left = left;
                    _self.setPosArr[moveIndex].top = top;
                    _self.setPosArr[moveIndex].posX =
                        (left + 10 + _self.signWidth / 2) / _self.scale;
                    _self.setPosArr[moveIndex].posY =
                        (top + 35 + _self.signWidth / 2) / _self.scale;
            ...
            imgWrapDom.on('mousedown', '.sign-wrap', function(e) {
                ...
                $(document).bind('mousemove', function(e) {
                    ...
                    _self.cloudSignList[dragCloudItemIndex].posx =
                        (left + target.width() / 2) / fileWInner;
                    ...
                    _self.cloudSignList[dragCloudItemIndex].y =
                        (top + target.outerHeight() / 2) / _self.scale;
        ...
        async submitSign() {
            ...
            // TODO 如果是审批流程，则等同于 `仅他人签署`
            if (this.isViaProcess) {
                let _data = await this.onlyOtherSign();

                _data = Object.assign(_data, {
                    approvalProcessId: this.processId
                });

                let res = await this.$api('urlInitiateContractApply').post(
                    'postNormal',
                    _data
                );
        ...
        async onlyOtherSign(ukeySignList = []) {
            ...
            let sendData = Object.assign(
                ...
                this.contractForm,
                {
                    ...
                    contractId: !!this.$route.query.renew
                        ? ''
                        : this.contractId || '',
                    isSiteSign:
            ...
            // 如果存在ukey印章，
            if (this.isFirstSign && ukeySignList.length > 0) {
                sendData.ukeySignReqVoS = ukeySignList;
            }

            // `审批流程`时，复用 `仅他人签署`的请求参数 `sendData`
            if (this.isViaProcess) {
                return sendData;
            }

            // 发起人是否需要签署
            let url = this.isFirstSign
